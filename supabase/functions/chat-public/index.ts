...
import { cacheGet, cachePut, checkBudget, distributedRateLimit, estimateTokens, logAudit, moderateInput, parseVerifiedWidgetToken, verifyWidgetToken } from "../_shared/enterprise-security.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const parsedSession = await parseVerifiedWidgetToken(String(session_token || ""));
    const user_id = parsedSession?.business_id || "";
    if (!demo && !user_id) {
      return new Response(
