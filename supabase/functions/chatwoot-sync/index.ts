...
async function syncProfile(userId: string) {
  ...
  const name = typeof metadata.full_name === "string"
    ...
    : typeof metadata.name === "string"
      ? metadata.name
      : email?.split("@")[0] ?? "CandyAI Customer";
...
async function syncTicket(userId: string, ticketId: string) {
  ...
  const conversation = await createConversation(config, contact.id, `candyai-ticket:${ticket.id}`, {
    candyai_ticket_id: ticket.id,
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "create_conversation") {
      if (auth === "internal") return json({ error: "internal requests cannot create conversations without a user token" }, 403);
      const config = getChatwootConfig();
    ...
    if (action === "send_message") {
      if (auth === "internal") return json({ error: "internal requests cannot send messages without a user token" }, 403);
      ...
      const config = getChatwootConfig();
      return json({ message: await sendMessage(config, conversationId, content, Boolean(body.private)) });
    }
