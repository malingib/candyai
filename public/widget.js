(function () {
  ...
  var css = `
    ...
    @keyframes mw-spin{to{transform:rotate(360deg)}} @keyframes mw-pulse{0%{box-shadow:0 0 0 0 rgba(var(--mw-primary-rgb, 37, 99, 235), 0.7)}70%{box-shadow:0 0 0 15px rgba(var(--mw-primary-rgb, 37, 99, 235), 0)}100%{box-shadow:0 0 0 0 rgba(var(--mw-primary-rgb, 37, 99, 235), 0)}}
  `;
  ...
  function renderActions() {
    var html = "";
    var waNum = theme.whatsapp ? sanitizePhone(theme.whatsapp).replace(/[^\d]/g, "") : "";
    ...
    if (waNum) {
      var context = messages.length > 1 ? "I'm continuing from the website chat." : "Hello, I'd like to inquire about your services.";
      html +=
        '<a class="mw-action-btn wa" target="_blank" rel="noopener" href="https://wa.me/' + encodeURIComponent(waNum) + '?text=' + encodeURIComponent(context) + '">' +
